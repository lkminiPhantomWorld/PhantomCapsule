...
| 件套 | 檔名 | 作用 |
|------|------|------|
| 1 | `NAMESPACE.json` | 宣告膠囊身份（`xmlns`）、命名空間、根節點、根協議 |
| 2 | `MANIFEST.json` | 列出該膠囊的必要檔案清單與關係 |
| 3 | `LOCATOR.json` | 記錄座標、協議、根節點指向、repo / branch / path |
| 4 | `PROTOCOL.json` | 輕量參考版，指向主協議；或直接引用 PhantomCapsule PROTOCOL.json |
| 5 | `SNAPSHOT.json` | 記錄當前膠囊狀態（可驗證） |
| 6 | `ReverseChain.json` | 記錄來源脈絡與版本演化 |
| 7 | `SHA256SUMS` | 對所有公開（或私有）膠囊檔案做雜湊驗證 |
| 8 | `PACKAGE.md` | 說明此膠囊的公開邊界、包含內容與排除內容 |
...
**缺少：** NAMESPACE.json、PROTOCOL.json、PACKAGE.md
...
需新增的 `PROTOCOL.json`（輕量參考版）：
```json
{
  "note": "此為輕量參考版。主協議定義見 lkminiPhantomWorld/PhantomCapsule/PROTOCOL.json。",
...
**現況：** 完整的 HTML 入口頁系統（最複雜）  
**缺少：** 八件套全部  
**注意：** 補齊件套時不得修改現有 HTML/JS 邏輯；件套放在根目錄，不干涉入口頁結構
...
## 所有膠囊共用的 PROTOCOL.json（輕量參考版）

除了 PhantomCapsule 本身（已有完整版），所有其他倉庫都用這個輕量版：
...
所有膠囊的 `NAMESPACE.json` 都必須包含相同的 `root_chain`，且最終回推至 `LKMINI://`。  
